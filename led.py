...
def spaced_lights(spacing, color_func):
...
def moving_gradient(period=0.25, cycle_duration = 2.0, refresh=0.01666):
    ...
    def morph_bg_color():
        global morph_pew_bg
        bg_color = hsv(morph_pew_bg, 1, 1)
        morph_pew_bg += refresh * 20 / cycle_duration * length
        morph_pew_bg %= 1
        return bg_color

    executor = Executor(refresh)
    executor.add(Wash(morph_bg_color))
    while True:
        executor.play(period)
...
def main_loop():
    ...
    while True:
        ...
        xmas_pew()
        pew_pew()
        choice_func = random.choice([random_fades, random_wipes, morph_pew, starfall])
        choice_func()
